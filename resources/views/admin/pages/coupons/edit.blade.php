    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <div class="flex items-center gap-2">
                <span class="text-lg font-extrabold text-slate-900">Edit Coupon: {{ $coupon->code }}</span>
                @if($coupon->isValid())
            ...
            <p class="text-xs text-slate-400">Total Redemptions so far: {{ $coupon->used_count }} orders</p>
        </div>
    </div>
@endsection
...
<div class="max-w-5xl mx-auto space-y-6 sm:space-y-8"
     x-data="{
         code: '{{ addslashes(old('code', $coupon->code)) }}',
         type: '{{ old('type', $coupon->type) }}',
         value: '{{ old('value', $coupon->value) }}',
         maxDiscount: '{{ old('max_discount_amount', $coupon->max_discount_amount) }}',
         minOrder: '{{ old('min_order_amount', $coupon->min_order_amount) }}',
         isActive: {{ old('is_active', $coupon->is_active ? 1 : 0) ? 'true' : 'false' }},
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-slate-200/80 p-5 sm:p-7 shadow-xs space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select id="type" 
                                ...
                                <option value="flat" {{ old('type', $coupon->type) == 'flat' ? 'selected' : '' }}>Flat Amount Discount (₹)</option>
                            </select>
                        </div>
                    ...
                    {{-- Max Discount Cap & Min Cart Value --}}
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-slate-200/80 p-5 sm:p-6 shadow-xs space-y-3">
                    <button type="submit" class="w-full py-3.5 px-6 rounded-xl text-xs font-extrabold text-white bg-slate-900 hover:bg-slate-800 transition-all shadow-sm hover:shadow-md flex justify-center items-center gap-2 cursor-pointer">
                        ...
                        <span>Update Coupon Details</span>
                    </button>
