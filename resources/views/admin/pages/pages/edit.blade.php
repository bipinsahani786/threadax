    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <div class="flex items-center gap-2">
                <span class="text-lg font-extrabold text-slate-900">Edit Page: {{ $page->title }}</span>
                @if($page->is_active)
            ...
            <p class="text-xs text-slate-400">Public URL: /page/{{ $page->slug }}</p>
        </div>
    </div>
@endsection
...
<div class="max-w-5xl mx-auto space-y-6 sm:space-y-8"
     x-data="{
         title: '{{ addslashes(old('title', $page->title)) }}',
         slug: '{{ addslashes(old('slug', $page->slug)) }}',
         metaTitle: '{{ addslashes(old('meta_title', $page->meta_title)) }}',
         metaDescription: '{{ addslashes(old('meta_description', $page->meta_description)) }}',
         isActive: {{ old('is_active', $page->is_active ? 1 : 0) ? 'true' : 'false' }}
     }">
    ...
    <div class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs">
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('frontend.page.show', $page->slug) }}" target="_blank" class="text-xs font-mono font-bold text-blue-600 hover:underline">
                https://threadax.co.in/page/{{ $page->slug }} ↗
            </a>
        </div>
        <span class="text-xs text-slate-400 font-medium">Last Modified: {{ $page->updated_at->diffForHumans() }}</span>
    </div>
                ...
                <div class="bg-white rounded-2xl border border-slate-200/80 p-5 sm:p-7 shadow-xs space-y-5">
                    ...
                    <div>
                        ...
                        <textarea id="body" 
                                  ...
                                  rows="14"
                                  required
                                  class="w-full p-4 bg-slate-50 hover:bg-slate-100/50 focus:bg-white border border-slate-200 focus:border-slate-400 rounded-xl text-xs font-mono font-medium text-slate-900 outline-none transition-all leading-relaxed @error('body') border-rose-500 @enderror">{{ old('body', $page->body) }}</textarea>
                ...
                <div class="bg-white rounded-2xl border border-slate-200/80 p-5 sm:p-7 shadow-xs space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="meta_description" 
                                  ...
                                  rows="2" 
                                  class="w-full p-3 bg-slate-50 hover:bg-slate-100/50 focus:bg-white border border-slate-200 focus:border-slate-400 rounded-xl text-xs font-medium text-slate-900 outline-none transition-all leading-relaxed">{{ old('meta_description', $page->meta_description) }}</textarea>
                    </div>
                </div>
