...
@section('page-title')
    <div class="flex items-center gap-3">
        ...
        <div>
            <span class="text-lg font-extrabold text-slate-900">Add Customer Testimonial</span>
            <p class="text-xs text-slate-400">Feature social proof, creator reviews, and buyer endorsements</p>
        </div>
    </div>
@endsection
...
<div class="max-w-5xl mx-auto space-y-6 sm:space-y-8"
     x-data="{
         ...
         designation: '{{ addslashes(old('designation', 'Verified Streetwear Buyer')) }}',
         content: `{{ addslashes(old('content', '')) }}`,
         rating: {{ old('rating', 5) }},
         photoUrl: '{{ addslashes(old('photo_url', '')) }}',
         videoUrl: '{{ addslashes(old('video_url', '')) }}',
                ...
                <div class="bg-white rounded-2xl border border-slate-200/80 p-5 sm:p-7 shadow-xs space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" 
                                   ...
                                   name="designation" 
                                   x-model="designation"
                                   value="{{ old('designation', 'Verified Streetwear Buyer') }}" 
                                   placeholder="e.g. Fashion Influencer, Verified Buyer"
                    ...
                    <div>
                        ...
                        <textarea id="content" 
                                  ...
                                  required
                                  maxlength="1000"
                                  placeholder="Describe the fabric quality, oversized fit, and brand experience..."
                                  class="w-full p-4 bg-slate-50 hover:bg-slate-100/50 focus:bg-white border border-slate-200 focus:border-slate-400 rounded-xl text-xs font-medium text-slate-900 outline-none transition-all leading-relaxed @error('content') border-rose-500 @enderror">{{ old('content') }}</textarea>
