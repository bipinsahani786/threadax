...
@section('content')
<form method="POST" action="{{ route('admin.testimonials.store') }}" class="max-w-2xl space-y-6">
    @csrf
    ...
    <div class="bg-white rounded-lg shadow p-6 space-y-5">
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-slate-400 focus:outline-none">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Designation</label>
                <input type="text" name="designation" value="{{ old('designation', 'Verified Buyer') }}" placeholder="e.g. Verified Buyer" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-slate-400 focus:outline-none">
            </div>
        </div>

        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Review *</label>
            <textarea name="content" rows="4" required maxlength="1000" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-slate-400 focus:outline-none">{{ old('content') }}</textarea>
        </div>
        ...
        <div>
            ...
            <input type="url" name="photo_url" value="{{ old('photo_url') }}" placeholder="https://..." class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-slate-400 focus:outline-none">
            <p class="text-xs text-gray-400 mt-1">Direct image URL (jpg, png, webp). Can use any hosted image.</p>
        </div>

        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Video URL (YouTube or MP4)</label>
            <input type="url" name="video_url" value="{{ old('video_url') }}" placeholder="https://www.youtube.com/watch?v=..." class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-slate-400 focus:outline-none">
            <p class="text-xs text-gray-400 mt-1">YouTube link or direct MP4 URL. Will show as a playable video testimonial.</p>
        </div>
    ...
    <div class="flex gap-3">
        <button type="submit" class="bg-slate-900 text-white text-sm font-semibold px-6 py-2.5 rounded-lg hover:bg-slate-700 transition-colors">Add Testimonial</button>
