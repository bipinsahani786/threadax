    ...
    <section class="py-12 lg:py-16 bg-brand-off-white/40">
        <div class="max-w-[1240px] mx-auto px-4 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
                ...
                @php
                    $contactEmail = $globalSettings['contact_email'] ?? 'support@threadax.co.in';
                    $contactPhone = $globalSettings['contact_phone'] ?? '+91 98765 43210';
                    $rawWhatsApp = $globalSettings['whatsapp_number'] ?? '919876543210';
                    $cleanWhatsApp = preg_replace('/[^0-9]/', '', $rawWhatsApp);
                    $cleanPhone = preg_replace('/[^0-9]/', '', $contactPhone);
                    $storeAddress = $globalSettings['store_address'] ?? 'ThreadAx HQ, Mumbai, Maharashtra, India';
                @endphp
                <div class="lg:col-span-5 space-y-4">
                    ...
                    {{-- 1. Phone / Mobile Support Card --}}
                    <div class="bg-white p-5 rounded-2xl shadow-sm border border-brand-border flex items-start gap-4">
                        <div class="w-10 h-10 rounded-xl bg-brand-dark text-white flex items-center justify-center shrink-0 shadow-sm text-lg">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                        </div>
                        <div>
                            <h3 class="font-heading font-bold text-brand-dark text-sm mb-0.5">Phone Support</h3>
                            <p class="text-brand-muted text-xs mb-2">Direct call desk for instant customer support.</p>
                            <a href="tel:+{{ $cleanPhone }}" class="text-brand-dark font-bold text-xs hover:underline flex items-center gap-1">
                                {{ $contactPhone }}
                    ...
                    <div class="bg-white p-5 rounded-2xl shadow-sm border border-brand-border flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <a href="https://wa.me/{{ $cleanWhatsApp }}?text={{ urlencode('Hello ThreadAX! I have a question regarding my order.') }}" target="_blank" class="text-emerald-600 font-bold text-xs hover:underline flex items-center gap-1">
                                Chat on WhatsApp (+{{ $cleanWhatsApp }})
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
                            </a>
                        </div>
                    </div>
                    ...
                    {{-- 3. Email Card --}}
                    <div class="bg-white p-5 rounded-2xl shadow-sm border border-brand-border flex items-start gap-4">
                        <div class="w-10 h-10 rounded-xl bg-brand-light text-brand-dark flex items-center justify-center shrink-0 shadow-sm">
                        ...
                        <div>
                            ...
                            <a href="mailto:{{ $contactEmail }}" class="text-brand-dark font-bold text-xs hover:underline flex items-center gap-1">
                                {{ $contactEmail }}
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
                            </a>
                        </div>
                    ...
                    {{-- 4. Business Hours & Registered Address --}}
                    <div class="bg-white p-5 rounded-2xl shadow-sm border border-brand-border flex items-start gap-4">
                        ...
                        <div>
                            ...
                            @if($storeAddress)
                                <p class="text-[11px] text-brand-muted mt-2 border-t border-brand-border/60 pt-2">
                                    <strong class="text-brand-dark">Store Office:</strong> {{ $storeAddress }}
                                </p>
                            @endif
                        </div>
                    </div>
